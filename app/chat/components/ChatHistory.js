...
const ChatHistory = () => {
  ...
  let globalIndex = 0; // <-- Global index for ref & truncation tracking
  ...
  return (
    <div className="mb-20">
        ...
        <div>
          ...
          {groupedMessages.today.map((msg) => {
            ...
            return (
              <div
                key={msg.id}
                className={`message-container flex items-center gap-2 my-1 ${truncatedMessages.includes(currentIndex) ? 'truncated' : ''}`}
                ref={(el) => (messageRefs.current[currentIndex] = el)}
              >
                {/* <span>{msg.messageJson}</span> */}
              </div>
            );
      ...
      {groupedMessages.last7Days.length > 0 && (
        <div>
          <h2 className="text-md font-semibold">Last 7 Days</h2>
          {groupedMessages.last7Days.map((msg) => {
            const currentIndex = globalIndex++;
            return (
              <div
                key={msg.id}
      ...
      {groupedMessages.last30Days.length > 0 && (
        <div>
          <h2 className="text-md font-semibold">Last 30 Days</h2>
          {groupedMessages.last30Days.map((msg) => {
            const currentIndex = globalIndex++;
            return (
              <div
                key={msg.id}
      ...
      {Object.keys(groupedMessages.previousMonths).map((monthYear) => (
        <div key={monthYear}>
          <h2 className="text-md font-semibold">{monthYear}</h2>
          {groupedMessages.previousMonths[monthYear].map((msg) => {
            const currentIndex = globalIndex++;
            return (
              <div
                key={msg.id}
